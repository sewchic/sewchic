...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function updateTimes() {
        // Get the current date and time in Auckland, New Zealand (UTC+13)
        const aucklandTime = new Date().toLocaleString('en-US', {
            timeZone: 'Pacific/Auckland'
        });

        // Get the <p> element by its ID for Auckland
        const aucklandTag = document.getElementById('aucklandDate');

        // Update the content of the <p> tag with the formatted current date and time in Auckland
        aucklandTag.textContent = formatDate(new Date(aucklandTime));

        // Get the current date and time in London, UK (UTC+0)
        const londonTime = new Date().toLocaleString('en-US', {
            timeZone: 'Europe/London'
        });

        // Get the <p> element by its ID for London
        const londonTag = document.getElementById('londonDate');

        // Update the content of the <p> tag with the formatted current date and time in London
        londonTag.textContent = formatDate(new Date(londonTime));
    }
...
parcelRequire = (function (modules, cache, entry, globalName) {
    ...
    "TNS6": [function (require, module, exports) {
        ...
        class OpacityEffect {
            ...
            mouseEnter() {
                this.siblingOpacity(0.3, 0.3);
            }

            mouseLeave() {
                this.siblingOpacity(1, 0.3);
            }

            siblingOpacity(opacityValue, easeValue) {
                const siblings = Array.from(this.DOM.el.parentNode.children).filter(child => child !== this.DOM.el);
                siblings.forEach(sibling => {
                    sibling.style.transition = `opacity ${easeValue}s`;
                    sibling.style.opacity = opacityValue;
                });
            }
        }
        ...
        // Create instances for each <a> tag with the class "award__item"
        document.querySelectorAll('.award__item').forEach(link => new OpacityEffect(link));

        document.querySelectorAll('.next__project--a').forEach(link => new OpacityEffect(link));

        document.querySelectorAll('.employment__history--link').forEach(link => new OpacityEffect(link));

        document.querySelectorAll('.info__details--row').forEach(link => new OpacityEffect(link));

        document.querySelectorAll('.project__details--row').forEach(link => new OpacityEffect(link));
    ...
    "QvaY": [function (require, module, exports) {
        ...
        var DOM = {
            splitcontent: {
                home: {
                    section: document.querySelector('.split__item--home'),

                    //                    get chars() {
                    //                        return this.section.querySelectorAll('.split__paragraph .word > .char, .whitespace');
                    //                    },

                    isVisible: false
                },
                about: {
                    section: document.querySelector('.split__item--about'),
                    ...
                    get picture() {
                        return document.querySelectorAll('.info__content');
                    },
            ...
            links: {
                ...
                home: document.querySelector('.frame__rose')
            }
        ...
        const opacityValue = isMobileDevice() ? 0 : 1; // Adjust opacity values as needed
        ...
        var timeline = _gsap.gsap.timeline({
            ...
            .set(DOM.splitcontent.about.picture, {
                y: '30%',
                rotation: -1,
                opacity: 0
            }, 'start') // Stagger the animation of the about section chars
        ...
        .to('.case_study--body', {
            ...
            .to(DOM.splitcontent.about.picture, {
                ease: 'Power3.easeOut',
                y: '0%',
                opacity: 1,
                pointerEvents: 'all',
                rotation: 0
            }, 'start+=0.5'); // Clicking the about and homepage links will toggle the content area, by playing and reversing the timeline
        // Need to switch current state for the about/close links


        var switchContent = function switchContent() {
            DOM.links.about.stateElement[0].classList[DOM.splitcontent.about.isVisible ? 'add' : 'remove']('frame__about-item--current');
            DOM.links.about.stateElement[1].classList[DOM.splitcontent.about.isVisible ? 'remove' : 'add']('frame__about-item--current');
            timeline[DOM.splitcontent.about.isVisible ? 'reverse' : 'play']();
            DOM.splitcontent.about.isVisible = !DOM.splitcontent.about.isVisible;
            DOM.splitcontent.home.isVisible = !DOM.splitcontent.about.isVisible;
        };
        ...
        DOM.links.home.addEventListener('click', function () {
            if (DOM.splitcontent.home.isVisible) return;
            switchContent();
        });
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const loadingScreen = document.body;

    // Total size of all assets to be loaded
    let totalAssetsSize = 0;

    // Size of loaded assets
    let loadedAssetsSize = 0;
    ...
    const hideLoadingScreen = () => {
        ...
        // Enable scrolling when the loader is hidden
        enableScroll();

        // Set initial state for elements in the Home section
        // DOM.splitcontent.home.section.classList.add('split__item--current');
        // DOM.splitcontent.about.section.classList.remove('split__item--current');
    ...
    const preloadAssets = (assets, callback) => {
        ...
        assets.forEach(asset => {
            console.log('starting to load' + asset)
            fetch(asset.url)
                .then(response => {
                    if (!response.ok) {
                ...
                .then(blob => {
                    loadedAssetsSize += asset.size || 0;
                    loadedCount++;

                    // Update the loading counter
                    updateLoadingCounter(loadedAssetsSize, totalAssetsSize);
                ...
                .catch(error => {
                    console.error(error);
                    loadedAssetsSize += asset.size || 0;
                    loadedCount++; // Treat the asset as loaded even in case of an error
    ...
    const updateLoadingCounter = (loaded, total) => {
        const counterElement = document.querySelector('.loading__counter');
        const targetPercentage = total > 0 ? Math.min(Math.round((loaded / total) * 100), 100) : 0;

        clearInterval(counterInterval);
        // Set an interval to increment the counter gradually
        counterInterval = setInterval(() => {
            counterElement.textContent = String(currentPercentage).padStart(3, '0');
            ...
            if (currentPercentage >= targetPercentage) {
                ...
                if (targetPercentage === 100) {
                    // Check if the minimum duration has passed before hiding the loading screen
                    checkMinimumDuration();
                }
            } else {
                currentPercentage += 1;
            }
        }, 10); // Adjust the interval as needed (40 milliseconds in this example)
    };

    // Function to check if the minimum duration has passed
    const checkMinimumDuration = () => {
        const minimumLoadingDuration = 3000; // in milliseconds
        ...
        if (elapsedTime >= minimumLoadingDuration) {
            ...
            const remainingTime = minimumLoadingDuration - elapsedTime;
            setTimeout(hideLoadingScreen, remainingTime);
        }
    };

    // Preload assets and start loading
    const commonAssets = [
        { url: './fonts/PPNeueMontreal-Regular.woff2', size: 48172 },
        { url: './fonts/PPNeueMontreal-Italic.woff2', size: 49_664 },
        { url: './fonts/PPNeueMontreal-Book.woff2', size: 50_840 },
        { url: './fonts/PPNeueMontreal-Medium.woff2', size: 52_996 },
        { url: './fonts/PPNeueMontreal-Bold.woff2', size: 53_672 },
        { url: './fonts/NaNHoloNarrow-Bold.woff2', size: 66_092 },
    ];

    const jeAssets = [
        { url: './img/je1.png', size: 810_216 },
        { url: './img/je2.png', size: 1_088_056 },
        { url: './img/je3.png', size: 2_778_371 },
        { url: './img/je4.png', size: 1_260_824 },
        { url: './img/je5.png', size: 1_337_233 },
        { url: './img/je6.png', size: 610_444 },
        { url: './img/je7.png', size: 1_207_536 },

        { url: './vid/je1.mp4', size: 1_939_291 },
        { url: './vid/je2.mp4', size: 4_684_614 },
        { url: './vid/je3.mp4', size: 373_682 },
        { url: './vid/je4.mp4', size: 4_035_413 },
    ];

    const ntAssets = [
        { url: './img/nt1.jpg', size: 53_069 },
        { url: './img/nt2.png', size: 118_360 },
        { url: './img/nt3.jpg', size: 140_229 },
        { url: './img/nt4.jpg', size: 58_629 },

        { url: './vid/ntv.gif', size: 118_023 },
        { url: './vid/ntv1.mp4', size: 12_004_599 },
        { url: './vid/ntv2.mp4', size: 7_091_530 },
        { url: './vid/ntv3.mp4', size: 1_098_232 },
        { url: './vid/ntv4.mp4', size: 9_811_143 },
        { url: './vid/ntv5.mp4', size: 3_449_291 },
    ];

    const ytAssets = [
        { url: './img/yt1.png', size: 833_970 },
        { url: './img/yt2.jpg', size: 188_957 },
        { url: './img/yt3.png', size: 333_660 },
        { url: './img/yt4.jpg', size: 986_944 },
        { url: './img/yt4b.png', size: 36_820 },
        { url: './img/yt5.png', size: 329_517 },
        { url: './img/yt6.jpg', size: 263_226 },
        { url: './img/yt7.png', size: 269_805 },
        { url: './img/yt8.png', size: 144_899 },

        { url: './vid/yt1.mp4', size: 18_409_327 },
        { url: './vid/yt2.mp4', size: 20_436_412 },
    ];



    // Check the page we are on and load those only assets along with the common assets
    let assetsToPreload = commonAssets
    switch(window.location.pathname) {
        case '/justegg.html':
            assetsToPreload = assetsToPreload.concat(jeAssets);
            break;
        case '/nurtalk.html':
            assetsToPreload = assetsToPreload.concat(ntAssets);
            break;
        case '/youtubegames.html':
            assetsToPreload = assetsToPreload.concat(ytAssets);
            break;
        default:
            console.error('Page not found, unsure what assets to preload, only loading commonAssets');
    }

    // Calculate the total size of all assets
    totalAssetsSize = assetsToPreload.reduce((total, asset) => total + asset.size, 0);

    // Preload assets and start loading
    preloadAssets(assetsToPreload, () => {
        // Your initialization code after all assets are loaded
        ...
        class SmoothScroll {
            ...
            style() {
                ...
                this.DOM.main.style.width = '100vw';
    ...
    links.forEach(function(link) {
        link.addEventListener('click', function(event) {
            ...
            var loaderIntro = document.querySelector('.loader-intro');
            loaderIntro.style.display = 'block';

            // Start animation
            loaderIntro.classList.add('slide-out');

            // When animation ends, redirect
            loaderIntro.addEventListener('animationend', function() {
                // Redirect to the clicked link's href
                window.location.href = linkHref;
            }, { once: true }); // Use once option to ensure the event listener is removed after being triggered
        });
    });
});
        ...
        "splitting/dist/splitting.css": "oR66",
        "splitting/dist/splitting-cells.css": "oR66",
        "splitting": "Qdhu",
        "gsap": "TpQl",
        "./utils": "MgTz",
        "./cursor": "LMRJ"
